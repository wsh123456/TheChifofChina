...
public class SimpleLOD_MaterialPopup : EditorWindow {
	...
	public new void Close() {
		...
		for(int i=0;i<submeshRows.Count;i++) {
			...
			Destroy((Texture2D)row["p0"]);
			Destroy((Texture2D)row["p1"]);
			Destroy((Texture2D)row["p2"]);
			Destroy((Texture2D)row["p3"]);
			Destroy((Texture2D)row["p4"]);
			Destroy((Texture2D)row["p5"]);
			Destroy((Texture2D)row["p6"]);
		}
		submeshRows.Clear();
		submeshRows = null;
		dragRow = null;
